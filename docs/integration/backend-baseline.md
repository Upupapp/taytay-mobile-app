...
| **F28** | **CLOSED, both sides** (`backend@c886eb6`) | **A KYC case had nowhere to put an identity document.** `POST me/kyc/documents` now attaches one, and the reviewer deciding the case can see and open it — without that half it would be a write nobody reads, which is the mistake F26 made one commit earlier. **No new table**: `Files` already owns slots, versioning, supersession, scan status and retention. Two types, closed; **no selfie, no liveness, no biometric**, because a facial image is not revocable the way a password is and a released build cannot grade its own verification. Accepted only while the case is the resident’s to change. The app offers them as *"documents you can send"*, never a checklist — a KYC case is adjudicated against the register first, and most residents are already in it. | closed |
